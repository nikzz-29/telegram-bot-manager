...
def _messages(locale: str) -> dict[str, str]:
    source = (LOCALES_DIR / locale / "main.ftl").read_text(encoding="utf-8")
    return {match.group(1): match.group(2) for match in _MESSAGE.finditer(source)}


def test_every_locale_ships_a_catalogue() -> None:
    for locale in SUPPORTED_LOCALES:
        assert (LOCALES_DIR / locale / "main.ftl").is_file(), locale
...
def test_locales_define_the_same_keys() -> None:
    ...
    """
    catalogues = {locale: set(_messages(locale)) for locale in SUPPORTED_LOCALES}
    reference = catalogues["ru"]
    for locale, keys in catalogues.items():
        assert keys == reference, f"{locale} differs: {keys ^ reference}"


def test_locales_agree_on_placeholders() -> None:
    ...
    """
    reference = {key: set(_PLACEABLE.findall(value)) for key, value in _messages("ru").items()}
    for locale in SUPPORTED_LOCALES:
