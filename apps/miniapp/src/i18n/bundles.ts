 ...
 * `dm.ftl`, `stats.ftl` and `guide.ftl` come along for the same reason: they are
 * the bot's other catalogues, and `i18n.runtime.BOT_CATALOGUES` is the list this
 * must mirror. A key defined in two of them resolves to whichever loads first here.
 */
...
import enStats from "@locales/en/stats.ftl?raw";
...
import ruStats from "@locales/ru/stats.ftl?raw";
...
const SOURCES: Record<Locale, readonly string[]> = {
  en: [enMain, enDm, enStats, enGuide, enPanel],
  ru: [ruMain, ruDm, ruStats, ruGuide, ruPanel],
};
